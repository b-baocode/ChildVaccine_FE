  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        const latestAppointment = await appointmentService.getLatedAppointmentsByChildId(selectedChild);
        ...
        <div className="form-group">
          ...
          {appointments.length > 0 ? (
            <div className="appointment-info">
              <p><strong>Ngày tiêm:</strong> {new Date(appointments[0].appointmentDate).toLocaleDateString()}</p>
              <p><strong>Mã cuộc hẹn:</strong> {appointments[0].appId}</p>
              <p><strong>Vaccine/Gói:</strong> {
                appointments[0].serviceName 
              }</p>
              <input 
                type="hidden" 
