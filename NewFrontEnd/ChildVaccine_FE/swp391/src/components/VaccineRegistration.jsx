...
const VaccineRegistration = () => {
    ...
    const [childrenWithActiveSchedules, setChildrenWithActiveSchedules] = useState([]);
    ...
    useEffect(() => {
        const checkChildrenSchedules = async () => {
          ...
          for (const child of childProfiles) {
            try {
              const schedules = await scheduleService.getSchedulesByChildId(child.childId);
              
              // Kiểm tra xem có schedule ACTIVE nào không
              const hasActiveSchedule = schedules.some(schedule => schedule.status === 'ACTIVE');
              
              if (hasActiveSchedule) {
                activeChildIds.push(child.childId);
              }
            } catch (error) {
              console.error(`Error checking schedules for child ${child.childId}:`, error);
            }
          }
    ...
    useEffect(() => {
        if (location.state && location.state.selectedItem) {
            ...
            setSelectedItemShots(selectedType === 'single' ? selectedItem.shotNumber : 1);
        }
    }, [location.state]);
    ...
    const SuccessModal = () => (
        <div className="success-modal">
            <div className="modal-content success">
                ...
                {formData.scheduleId && (
                    <p>Mã cuộc hẹn: <strong>{formData.scheduleId}</strong></p>
                )}
                <button
            ...
            >
              <option value="">
                {availableChildren.length === 0 
                  ? '-- Không có hồ sơ trẻ có thể đăng ký --' 
                  : '-- Chọn hồ sơ trẻ --'}
              </option>
